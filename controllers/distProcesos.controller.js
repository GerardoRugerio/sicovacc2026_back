...
export const DatosDistrito = async (req = request, res = response) => {
    ...
    try {
        const datos = (await SICOVACC.sequelize.query(`SELECT CCDe.nombre_delegacion, CCDi.domicilio, CCDi.cp AS codigo_postal, COALESCE(CCDi.coordinador, '') AS coordinador, COALESCE(CCDi.coordinador_puesto, '') AS coordinador_puesto, COALESCE(CCDi.coordinador_genero, '') AS coordinador_genero,
        COALESCE(CCDi.secretario, '') AS secretario, COALESCE(CCDi.secretario_puesto, '') AS secretario_puesto, COALESCE(CCDi.secretario_genero, '') AS secretario_genero
...
export const ActualizarDatosDistrito = async (req = request, res = response) => {
    ...
    try {
        await SICOVACC.sequelize.query(`UPDATE consulta_cat_distrito SET domicilio = '${Comillas(domicilio)}', coordinador = ${coordinador ? `'${Comillas(coordinador)}'` : 'NULL'}, coordinador_puesto = ${coordinador_puesto ? `'${Comillas(coordinador_puesto)}'` : 'NULL'}, coordinador_genero = ${coordinador_genero ? `'${coordinador_genero}'` : 'NULL'},
        secretario = ${secretario ? `'${Comillas(secretario)}'` : 'NULL'}, secretario_puesto = ${secretario_puesto ? `'${Comillas(secretario_puesto)}'` : 'NULL'}, secretario_genero = ${secretario_genero ? `'${secretario_genero}'` : 'NULL'}, fecha_modif = CURRENT_TIMESTAMP WHERE id_distrito = ${id_distrito}`);
