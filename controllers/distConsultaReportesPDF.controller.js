...
export const ProyectosParticipantes = async (req = request, res = response) => {
    ...
    try {
        const consulta = (await SICOVACC.sequelize.query(`SELECT secuencial, folio, destino_recursos_final, nom_proyecto
...
export const ActaValidacionPDF = async (req = request, res = response) => {
    ...
    try {
        ...
        res.json({
            ...
            buffer: (await streamToBuffer(await GeneradorActaPDF(anio, id_distrito, clave_colonia)))
        });
    } catch (err) {
...
export const ActasValidacionZip = async (req = request, res = response) => {
    ...
    try {
        ...
        const { id } = isProd ? await BuscarUsuarioID(id_transaccion) : BuscarUsuarioID(id_transaccion);
        const { getSocketServerInstance } = await import(`../sockets/sockets${sufix}.js`);
        const io = getSocketServerInstance();
        const generarZipBuffer = async (colonias) => {
            return new Promise((resolve, reject) => {
                const archive = archiver('zip', { zlib: { level: 9 } });
                const buffer = [];
                const limit = pLimit(colonias.length > 30 ? 3 : 5);
                let procesadas = 0, faseActual = 'generando';
                archive.on('progress', progress => {
                    if (faseActual !== 'comprimiendo') return;
                    const { total, processed } = progress.entries;
                    const porcentaje = Math.round((processed / total) * 100);
                    io.to(id).emit('descarga-zip', {
                        fase: 'comprimiendo',
                        porcentaje
                    });
                });
                ...
                (async () => {
                    try {
                        await Promise.all(
                            colonias.map(clave_colonia =>
                                limit(async () => {
                                    const stream = await GeneradorActaPDF(anio, id_distrito, clave_colonia);
                                    return new Promise((resolve, reject) => {
                                        stream.on('end', resolve);
                                        stream.on('error', reject);
                                        archive.append(stream, { name: `ActaValidacion_${clave_colonia}_${anio == 2 ? '2026' : '2027'}_${fechaM}_${horaM}.pdf` });
                                        procesadas++;
                                        const porcentaje = Math.round((procesadas / colonias.length) * 100);
                                        io.to(id).emit('descarga-zip', {
                                            fase: 'generando',
                                            porcentaje
                                        });
